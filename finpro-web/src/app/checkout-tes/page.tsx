    ...
    <main className="min-h-screen bg-white flex flex-col items-center justify-between">
      <div className="w-full max-w-6xl md:px-5">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-1">
                {mainAddress ? (
                  <>
                    ...
                    <p>{user?.firstName} - {user?.phoneNumber}</p>
                    <p>{mainAddress.subDistrict}, {mainAddress.district}, {mainAddress.city}, {mainAddress.province} {mainAddress.postalCode}</p>
                  </>
                ) : (
                  <p className="text-red-600">Belum ada alamat utama. Silakan tambah alamat.</p>
                )}
              </div>
            </div>
              ...
              <div className="text-gray-500 space-y-1">
                ...
                <div className="flex items-center gap-2 w-full">
                  ...
                  <p className="text-sm flex-1 whitespace-normal">
                    Maks. 1 jam setelah pembayaran selama jam operasional (07:00 - 21:00)
                  </p>
                </div>
              </div>
            </div>
          ...
          {/* RIGHT: Ringkasan dan Total */}
          <div className="hidden md:block bg-white border rounded-lg p-6 shadow-sm">
            <h2 className="text-xl font-semibold text-gray-700 mb-7">Ringkasan Pembayaran</h2>
            <div className="space-y-4 text-black">
              <div className="flex justify-between">
                <span>Subtotal</span>
                <span>Rp {order?.totalAmount?.toLocaleString("id-ID") || totalBelanja.toLocaleString("id-ID")}</span>
              </div>
              <div className="flex justify-between">
                <span>Diskon</span>
                <span>Rp 0</span>
              ...
              <div className="flex justify-between">
                ...
                <span>Rp {order?.finalShippingCost?.toLocaleString("id-ID") || 0}</span>
              </div>
            </div>
            ...
            <div className="text-black font-bold">
              <div className="flex justify-between">
                ...
                <span>Rp {order?.finalTotalAmount?.toLocaleString("id-ID") || totalBelanja.toLocaleString("id-ID")}</span>
              </div>
            </div>
            <button
